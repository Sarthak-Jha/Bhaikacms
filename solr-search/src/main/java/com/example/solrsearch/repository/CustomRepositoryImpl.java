...
public class CustomRepositoryImpl implements CustomRepository {
    ...
    private static final String VIDEO_COLLECTION = "episode";
    @Resource
    ...
    @Override
    public Page<Video> search(String searchTerm, String videoType, String categoryFilter, Pageable var1) {
        SimpleQuery solrQuery = new SimpleQuery(searchTerm);
        if (videoType != null) {
            solrQuery.addFilterQuery(new SimpleFilterQuery(new Criteria("categoriesList").is(categoryFilter)))
                    .addFilterQuery(new SimpleFilterQuery(new Criteria("programType").is(videoType)))
        ...
        Page<Video> results
                = solrTemplate.query(VIDEO_COLLECTION, solrQuery, Video.class);
        return results;
    }
